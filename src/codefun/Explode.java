...
public static void explode(View aView)
{
    ...
    for(int i=0;i<10;i++)
        for(int j=0;j<10;j++)
            explodePiece(aView, i, j);
    
    // Hide view and make visible after delay
    aView.setOpacity(0);
    aView.getAnimCleared(2000).getAnim(2800).setOpacity(1).play();
}

public static void explodePiece(View aView, int aRow, int aCol)
{
    // Get bounds of piece
    ...
    ViewAnim anim = iview.getAnim(time);
